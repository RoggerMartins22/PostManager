...
async def update_status(id: int, request: UpdateStatusRequest, db: Session = Depends(get_db)):
    return update_status_postagem(db=db, postagem_id=id, statusPostagem=request.status, mensagem=request.mensagem)